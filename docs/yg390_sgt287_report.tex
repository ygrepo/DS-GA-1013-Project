...
value decomposition. The decay of the spectral spectrum $\sigma_k, k \in \mathcal{N}$ of  $\mathcal{A}$ is strongly related to the ill-posedness: faster decay implies a more ill-posed problem  \ref{fig:svd}. 
...
\centering
   ...
   \centering
		\includegraphics[width=\textwidth]{variance_plot_log.pdf}
		\caption{$50$ first singular value of the k-space of an image \\ from the fastMRI dataset after undersampling.}
		\label{fig:svd}
    \end{subfigure}%
   \begin{subfigure}[]{.5\textwidth}
   \centering   
      		\includegraphics[width=\textwidth]{nn_architecture}
...
\end{align*}
The last equation has a natural recursive form, which is easily interpretable as a deep learning architecture  \ref{fig:nn_architecture}:
\begin{align*}
